...
-- =============================================================================
-- BAGIAN 1: Drop VIEWS yang akan diubah menjadi TABLE
-- (Di magnum_sales objek-objek ini adalah VIEW, di svelte_go berupa TABLE)
-- =============================================================================
DROP VIEW IF EXISTS `followup_notification`;
DROP VIEW IF EXISTS `get_pivot_cancel_quotation`;
DROP VIEW IF EXISTS `get_pivot_decline_quotation`;
DROP VIEW IF EXISTS `get_pivot_followup_quotation`;
DROP VIEW IF EXISTS `get_pivot_po_quotation`;
DROP VIEW IF EXISTS `get_quotation_year_to_date`;
DROP VIEW IF EXISTS `get_sales_project_retail`;

-- =============================================================================
-- BAGIAN 2: Drop INDEX tambahan dari magnum_sales yang tidak ada di svelte_go
-- =============================================================================

-- customer: svelte_go hanya punya PRIMARY KEY (id)
ALTER TABLE `customer` DROP INDEX `name`;
ALTER TABLE `customer` DROP INDEX `category_id`;
ALTER TABLE `customer` DROP INDEX `sales_id`;

-- customer_contact: svelte_go hanya punya PRIMARY KEY (id)
ALTER TABLE `customer_contact` DROP INDEX `cust_contact_name`;

-- master_departements: svelte_go hanya punya PRIMARY KEY (id)
ALTER TABLE `master_departements` DROP INDEX `name_property`;

-- master_properties: svelte_go tidak punya index khusus
ALTER TABLE `master_properties` DROP INDEX `name`;
ALTER TABLE `master_properties` DROP INDEX `code`;

-- payment_term: svelte_go hanya punya PRIMARY KEY (id)
ALTER TABLE `payment_term` DROP INDEX `property_id`;

-- quotation: svelte_go punya PRIMARY KEY (id) saja
-- Di magnum_sales: PRIMARY KEY (property_id, id) -> akan diubah
ALTER TABLE `quotation` DROP INDEX `id`;
ALTER TABLE `quotation` DROP INDEX `fromdate_todate`;

-- quotation_detail: svelte_go: PRIMARY KEY (id, rev_id, line)
-- magnum_sales: PRIMARY KEY (rev_id, id, line) -> urutan berbeda, akan diubah nanti

-- quotation_files: svelte_go tidak punya index
ALTER TABLE `quotation_files` DROP INDEX `id`;

-- quotation_followup: svelte_go tidak punya index
ALTER TABLE `quotation_followup` DROP INDEX `id`;

-- quotation_status: svelte_go hanya punya PRIMARY KEY (id)
ALTER TABLE `quotation_status` DROP INDEX `name`;

-- setting: svelte_go tidak punya index
ALTER TABLE `setting` DROP INDEX `setting_code`;

-- units: svelte_go hanya punya PRIMARY KEY (id)
ALTER TABLE `units` DROP INDEX `name`;

-- user_group_policies: svelte_go punya PRIMARY KEY (id), KEY fk_table_access (table_id)
ALTER TABLE `user_group_policies` DROP INDEX `tblname`;
ALTER TABLE `user_group_policies` DROP INDEX `group_id`;

-- vendor: svelte_go tidak punya index khusus
ALTER TABLE `vendor` DROP INDEX `name`;

-- =============================================================================
-- BAGIAN 3: Ubah PRIMARY KEY tabel tertentu
-- =============================================================================

-- quotation: dari PRIMARY KEY (property_id, id) menjadi PRIMARY KEY (id)
ALTER TABLE `quotation` DROP PRIMARY KEY;
-- Pastikan id NOT NULL (sudah)
ALTER TABLE `quotation` ADD PRIMARY KEY (`id`);

-- quotation_detail: dari PRIMARY KEY (rev_id, id, line) menjadi PRIMARY KEY (id, rev_id, line)
ALTER TABLE `quotation_detail` DROP PRIMARY KEY;
ALTER TABLE `quotation_detail` ADD PRIMARY KEY (`id`, `rev_id`, `line`);

-- quotation_followup: dari PRIMARY KEY (property_id, id, line_id) -> tanpa PRIMARY KEY
ALTER TABLE `quotation_followup` DROP PRIMARY KEY;
...
-- setting: dari PRIMARY KEY (id) -> tanpa PRIMARY KEY
ALTER TABLE `setting` DROP PRIMARY KEY;

-- user_policies: dari PRIMARY KEY (id) -> tanpa PRIMARY KEY
ALTER TABLE `user_policies` DROP PRIMARY KEY;

-- vendor: dari PRIMARY KEY (id) -> tanpa PRIMARY KEY
ALTER TABLE `vendor` DROP PRIMARY KEY;

-- master_properties: dari PRIMARY KEY (id) -> tanpa PRIMARY KEY
ALTER TABLE `master_properties` DROP PRIMARY KEY;

-- quotation_files: dari (tidak ada PK di svelte_go) -> tanpa PRIMARY KEY
ALTER TABLE `quotation_files` DROP PRIMARY KEY;
...
-- -----------------------------------------------------
-- Tabel: get_pivot_cancel_quotation
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `get_pivot_cancel_quotation` (
...
-- -----------------------------------------------------
-- Tabel: get_pivot_decline_quotation
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `get_pivot_decline_quotation` (
  `April` double DEFAULT NULL,
  `August` double DEFAULT NULL,
  `December` double DEFAULT NULL,
  `February` double DEFAULT NULL,
...
-- -----------------------------------------------------
-- Tabel: get_pivot_followup_quotation
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `get_pivot_followup_quotation` (
  `April` double DEFAULT NULL,
  `August` double DEFAULT NULL,
  `December` double DEFAULT NULL,
  `Febuary` double DEFAULT NULL,
  `January` double DEFAULT NULL,
  `July` double DEFAULT NULL,
  `June` double DEFAULT NULL,
  `March` double DEFAULT NULL,
...
-- -----------------------------------------------------
-- Tabel: get_pivot_po_quotation
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `get_pivot_po_quotation` (
  `April` double DEFAULT NULL,
  `August` double DEFAULT NULL,
  `December` double DEFAULT NULL,
  `Febuary` double DEFAULT NULL,
...
-- -----------------------------------------------------
-- Tabel: get_quotation_year_to_date
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `get_quotation_year_to_date` (
  `name` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `total` double DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- -----------------------------------------------------
-- Tabel: get_sales_project_retail
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `get_sales_project_retail` (
  `quotation_type` int DEFAULT NULL,
  `total` double DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- =============================================================================
-- BAGIAN 7: Tambah INDEX yang ada di svelte_go
-- =============================================================================

-- counter_id: sudah sama

-- currency: sudah sama (PRIMARY KEY id, UNIQUE KEY name)

-- customer: PRIMARY KEY (id) [sudah]

-- customer_category: PRIMARY KEY (id), UNIQUE KEY name [sudah]

-- customer_contact: PRIMARY KEY (id) [sudah]

-- master_departements: PRIMARY KEY (id) [sudah]

-- master_table_access: PRIMARY KEY (id) [sudah]

-- payment_term: PRIMARY KEY (id) [sudah]

-- project_level: PRIMARY KEY (id) [sudah]

-- project_priority: PRIMARY KEY (id) [sudah]

-- quotation: PRIMARY KEY (id) [sudah diubah di Bagian 3]

-- quotation_detail: PRIMARY KEY (id, rev_id, line) [sudah diubah]

-- quotation_master: PRIMARY KEY (id, rev_id) [sudah sama]

-- quotation_progress: PRIMARY KEY (id) [sudah]

-- quotation_status: PRIMARY KEY (id) [sudah]

-- quotation_subdetail: PRIMARY KEY (id, rev_id, line, subline) [sudah diubah]

-- units: PRIMARY KEY (id) [sudah]

-- users: PRIMARY KEY (id), UNIQUE KEY email, UNIQUE KEY idx_users_email
-- Di magnum_sales: UNIQUE KEY users_email_unique -> ganti nama
